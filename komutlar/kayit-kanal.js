...
.setColor(`${renk}`)
.setDescription(`${red} Kayıt Olunacak Kanalı Belirtiniz !  `)
.setThumbnail(client.user.avatarURL())
.setFooter(`Komut ${message.author.tag} Tarafından Kullanıldı ! `)
message.channel.send(ayarlanmadı)
}
kayıtverileri.set(`kayıtkanal.${message.guild.id}`, kanal.id)
const ayarlandı = new discord.MessageEmbed()
.setAuthor(client.user.username, client.user.avatarURL())  
.setTitle(`${client.user.username} - Kayıt Kanal Ayarlandı `)
.setColor(`${renk}`)
.setDescription(`${onay} Kayıt Olunacak Kanal ${kanal} Olarak Ayarlandı | \`${prefix}kayıt-kanal sıfırla\` `)
.setThumbnail(client.user.avatarURL())
.setFooter(`Komut ${message.author.tag} Tarafından Kullanıldı ! `)
